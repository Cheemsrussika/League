...
# These are now INTERNAL variables, not @exports
...
@onready var skill_q: SkillSlot = $Skills/Skill_Q # Make sure you have a Skills Node with these slots!
...
func _ready():
	...
	if current_champion_data:
		# We check type here instead of in the argument to avoid the Scope Error
		if current_champion_data is ChampionData:
...
func initialize_from_data(data: ChampionData) -> void:
	...
	match resource_type:
		...
		_: current_resource = data.base_resource # Mana/Energy start full
	recalculate_stats() # Force update everything once loaded
