...
enum UnitClassification { NORMAL, ELITE, BOSS, EPIC }
enum Team { BLUE, RED, NEUTRAL, PURPLE }

@export var unit_type: UnitType = UnitType.MINION
@export var unit_classification: UnitClassification = UnitClassification.NORMAL
...
# ==========================================
# 1. ENGINE CALLBACKS (Lifecycle & Input)
# ==========================================
# Put Godot's built-in virtual functions at the top so it's easy to see 
# exactly what this script is executing frame-by-frame or on startup.
...
# ==========================================
# 2. STAT CALCULATION & MANAGEMENT
# ==========================================
# Functions related to calculating, getting, modifying, and regenerating stats.
...
func get_total(s: Stat) -> float:
	...
	# Special Logic for Health/Mana Regen
			...
			if item:
				if "stats" in item:
					for stat_key in item.stats:
						stat_modifiers[stat_key] = stat_modifiers.get(stat_key, 0.0) + item.stats[stat_key]
				if "effects" in item and item.effects:
					for effect in item.effects:
						if effect.has_method("on_stat_calculation"):
							effect.on_stat_calculation(self)

	# Read Stats from the Status Container
	if status_container:
			...
			if "stats_to_buff" in status:
				for stat_enum in status.stats_to_buff:
					if STAT_MAP.has(stat_enum):
						var string_key = STAT_MAP[stat_enum]
...
# ==========================================
# Core gameplay loop functions for hitting, dying, and triggering death rules.
...
# 4. SHIELD SYSTEM
# ==========================================
# Specialized logic dedicated strictly to generating, decaying, and breaking shields.
...
# ==========================================
# 5. STATUS EFFECTS & PASSIVES
# ==========================================
# Code governing buff/debuff instantiation, stacking, and dispatching global triggers.
...
# 6. UTILITIES & HELPERS
# ==========================================
# Small functions to read basic data checks.

func has_tag(tag_name: String) -> bool:
	return unit_tags.has(tag_name)
